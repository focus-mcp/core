...
function validateTool(raw: unknown): ToolDefinition {
    ...
    if (typeof name !== 'string' || name.length === 0) {
        throw new ManifestError('Tool.name must be a non-empty string', 'INVALID_TOOL', {
            tool: rec,
        });
    }
    if (/[^a-z0-9]/.test(name)) {
...
function validateConfigField(key: string, raw: unknown): ConfigField {
    ...
    if (rec['required'] !== undefined) {
        if (typeof rec['required'] !== 'boolean') {
            throw new ManifestError(
                `Config field "${key}".required must be boolean`,
                'INVALID_CONFIG',
                {
                    field: key,
                },
...
function validateTags(raw: unknown): readonly string[] {
    ...
    for (const tag of raw) {
        if (typeof tag !== 'string' || tag.length === 0) {
            throw new ManifestError(
                'Each manifest tag must be a non-empty string',
                'INVALID_TAGS',
                {
                    tag,
                },
